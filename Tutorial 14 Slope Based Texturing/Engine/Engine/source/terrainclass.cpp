...
bool TerrainClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* heightMapFilename, char* grassTextureFilename, char* slopeTextureFilename, 
							  char* rockTextureFilename)
{
	bool result;
	...
	// Load the textures.
	result = LoadTextures(device, deviceContext, grassTextureFilename, slopeTextureFilename, rockTextureFilename);
	if(!result)
	{
		return false;
	}
...
bool TerrainClass::LoadTextures(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* grassTextureFilename, char* slopeTextureFilename, char* rockTextureFilename)
{
	bool result;
	...
	result = m_GrassTexture->Initialize(device, deviceContext, grassTextureFilename);
	if(!result)
	{
		return false;
	}
	...
	result = m_SlopeTexture->Initialize(device, deviceContext, slopeTextureFilename);
	if(!result)
	{
		return false;
	}
	...
	result = m_RockTexture->Initialize(device, deviceContext, rockTextureFilename);
	if(!result)
	{
		return false;
	}
